...
(function ($) {
    ...
    function hoursMinutesSeconds($this, opts) {
        ...
        if (window['minutes_HMS' + id] == opts.minutes && window['seconds_HMS' + id] == opts.seconds && window['hours_HMS' + id] == opts.hours) {
            ...
            var upSec = ( parseInt(window['hours_HMS' + id]) * 60 * 60) + ( parseInt(window['minutes_HMS' + id]) * 60 ) + parseInt(window['seconds_HMS' + id] )
            //console.log(upSec);
            if( upSec < opts.alertFromSec ) {
                html($this, '<span class="blink_me">'+window['hours_HMS' + id] + opts.timeSeparator + window['minutes_HMS' + id] + opts.timeSeparator + window['seconds_HMS' + id]+'</span>');
            } else {
                html($this, window['hours_HMS' + id] + opts.timeSeparator + window['minutes_HMS' + id] + opts.timeSeparator + window['seconds_HMS' + id]);
            }
